          ...
          {cart.map(item => (
            <div key={(item.product.id + (item.variation?.id ? `-${item.variation.id}` : ''))} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 mb-2">৳{formatMoney(getCartItemUnitPrice(item))} / unit</p>
              ...
              <div className="text-right hidden sm:block">
                <span className="font-black text-lg">৳{formatMoney(getCartItemUnitPrice(item) * toFiniteNumber(item.quantity))}</span>
              </div>
            </div>
          ))}
